...
export function createCheckoutActions(page: Page) {
  return {
    ...
    async fillCustomerForm(customer: {
      ...
      await page.getByLabel("Nome", { exact: true }).fill(customer.name);
      await page.getByLabel("Sobrenome", { exact: true }).fill(customer.surname);
      const emailInput = page.getByLabel("Email", { exact: true });
      await emailInput.fill(customer.email);
      await emailInput.blur();
      await page.getByLabel("Telefone", { exact: true }).fill(customer.phone);
      const cpfInput = page.getByLabel("CPF", { exact: true });
      await cpfInput.fill(customer.cpf);
      await cpfInput.blur();
      await page.locator("button").filter({ hasText: "Selecione uma loja" }).click();
      await page.getByText(customer.store).last().click();
    },
